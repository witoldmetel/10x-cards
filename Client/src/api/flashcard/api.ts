...
export async function getFlashcards(
  ...
  const { data } = await instance.get<PaginatedResponse<Flashcard>>(
...
export async function getArchivedFlashcards(
  params?: FlashcardsQueryParams
): Promise<PaginatedResponse<Flashcard>> {
  const { data } = await instance.get<PaginatedResponse<Flashcard>>(
    `${FLASHCARD_API_ROUTES.GET_FLASHCARDS}/archived`,
    { params }
  );
  return data;
}
...
export async function updateFlashcard(
  ...
  const { data } = await instance.put<Flashcard>(
    `${FLASHCARD_API_ROUTES.GET_FLASHCARDS}/${id}`,
    flashcard
  );
  return data;
}

export async function batchUpdateFlashcards(
  request: BatchUpdateRequest
): Promise<BatchUpdateResponse> {
  const { data } = await instance.put<BatchUpdateResponse>(
    `${FLASHCARD_API_ROUTES.GET_FLASHCARDS}/batch`,
    request
  );
  return data;
}
...
export async function archiveFlashcard(id: string): Promise<Flashcard> {
  const { data } = await instance.put<Flashcard>(
    `${FLASHCARD_API_ROUTES.GET_FLASHCARDS}/${id}/archive`
  );
  return data;
}
...
export async function unarchiveFlashcard(id: string): Promise<Flashcard> {
  const { data } = await instance.put<Flashcard>(
    `${FLASHCARD_API_ROUTES.GET_FLASHCARDS}/${id}/unarchive`
  );
  return data;
}
